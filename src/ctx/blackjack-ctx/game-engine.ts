...
export class BlackjackEngine {
  ...
  calculateHandValue(cards: Card[]): { value: number; isSoft: boolean; isBlackjack: boolean; isBust: boolean } {
    ...
    const isSoft = aces > 0 && value <= 21 && cards.some(card => 
      card.rank === 'A' && value - 10 >= 1
    );
  ...
  determineWinner(playerHand: Hand, dealerHand: Hand): 'player-wins' | 'dealer-wins' | 'push' | 'player-blackjack' | 'dealer-blackjack' {
